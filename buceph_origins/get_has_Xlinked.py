...
with open(sys.argv[1], 'r') as gff_file:
	...
	for line in gff_file:
		...
		if splitline[0] == "X" and splitline[2] == "CDS":
			...
			if lower_coord > 71 and upper_coord < 261:
				gene_ID = splitline[-1]
				
				if gene_ID not in Xlinked_genes:
					Xlinked_genes.append(splitline[-1])
...
with open(sys.argv[2], 'r') as cds_file:
	...
	for line in cds_file:
		if line.startswith(">"):
			...
			if any(gene_ID in string for string in Xlinked_genes):
				print(line.strip())

				if len(used_genes) == 0:

					used_genes.append(gene_ID)
					complement_index = [i for i, s in enumerate(Xlinked_genes) if gene_ID in s][0]
